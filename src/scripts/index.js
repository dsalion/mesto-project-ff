...
closeBtns.forEach((el) => {el.addEventListener("click", () => { const popup = el.closest(".popup"); closePopup(popup)})});
...
buttonAddCard.addEventListener("click", () =>  {changingButtonStatus(formTypePlace, false); openAddCard()});
editProfileButton.addEventListener("click", () => { changingButtonStatus(formTypeProfile, false); openEditProfile()});
profileAvatarBtn.addEventListener('click', () => {changingButtonStatus(formTypeAvatar, false);openModalNewAvatar()});
